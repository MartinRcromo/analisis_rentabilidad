...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. Llamar a la función SQL que calcula todo en el servidor
    const { data: proveedoresData, error: errorRpc } = await supabase.rpc('calcular_proveedores', {
      p_periodo: periodoActual,
    });

    if (errorRpc) {
      console.error('Error en RPC calcular_proveedores:', errorRpc);
      return NextResponse.json({ error: 'Error calculando proveedores', details: errorRpc.message }, { status: 500 });
    }

    // 3. Filtrar por empresa si se especificó
    let proveedoresFiltrados = proveedoresData || [];
    if (empresa && empresa !== 'todas') {
      proveedoresFiltrados = proveedoresFiltrados.filter((s: { empresa: string }) => s.empresa === empresa);
    }

    // 4. Calcular totales para el response
    const totales = proveedoresFiltrados.reduce(
    ...
    return NextResponse.json({
      ...
      totales: {
        ...totales,
        total_proveedores: proveedoresFiltrados.length,
        proveedores_perdida: proveedoresFiltrados.filter((s: { en_perdida: boolean }) => s.en_perdida).length,
      },
      gastos: {
